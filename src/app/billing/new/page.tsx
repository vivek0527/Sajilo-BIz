  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
                  ...
                  {lineItems.map((item) => (
                    <div key={item.id} className="p-4 space-y-2">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-mono font-semibold text-sm">{currencySymbol}{(item.quantity * item.unit_price).toFixed(2)}</span>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {/* RIGHT COLUMN: LIVE BILL SETTLEMENT PREVIEW (Col Span 1) */}
        <div className="space-y-6">
          <div className="glass-panel rounded-2xl border border-border bg-gradient-to-br from-card to-secondary/20 p-6 space-y-6 shadow-lg relative overflow-hidden">
            {/* Sparkle decoration */}
            <div className="absolute top-0 right-0 h-24 w-24 rounded-full bg-primary/5 blur-xl -z-10" />

            <div className="border-b border-border pb-4 flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                <Receipt size={14} className="text-primary" /> Invoice Settlement
              </span>
            ...
            <button
              ...
              disabled={createBillMutation.isPending || lineItems.length === 0}
              className="w-full flex items-center justify-center gap-2 rounded-xl bg-primary py-3.5 text-sm font-bold text-primary-foreground shadow-lg shadow-primary/25 hover:bg-primary/95 disabled:opacity-50 disabled:pointer-events-none transition-all"
            >
              <Printer size={18} /> Process Bill & Checkout <ChevronRight size={16} />
            </button>
          </div>
        </div>
      </div>
